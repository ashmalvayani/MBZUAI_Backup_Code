...
# invisible_btn = gr.Button(interactive=False)
...
RAG = RAGPretrainedModel.from_index(f'{current_dir}/RAG/UAE_Documents')
...
def add_text(state, model_selector, text, request: gr.Request):
    ...
    ## Change it later so only first message is displayed, but will have to change it in conversation.py file
    # state = None
    if state is None:
    ...
    check_templateQA = template_questions(text)
    if check_templateQA != 'Question not found':
        logger.info(f"Template message detected. Text: {text}")
        conv.append_message(conv.roles[0], text)
        conv.append_message(conv.roles[1], check_templateQA)
        state.skip_next = True
        return (state, state.to_gradio_chatbot(), "") + (disable_btn,) * 5

    # making changes here
    result_docs = RAG.search(query=text, k=1)
...
block_css = """
.gradio-container {
    background: url('file=background_logo4.png');
...
def build_single_model_ui(models, add_promotion_links=False):
    with gr.Blocks():
        ...
        # 🌄🌄 GeoMin-GPT: A Guide to UAE’s Geology and Mineral Wonders and Beyond
        notice_markdown = f"""
        🌄🌄 GeoMin-GPT​
        """
